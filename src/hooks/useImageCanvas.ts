...
export function useImageCanvas({
  ...
  const handleImageLoad = useCallback((image: HTMLImageElement, canvas: HTMLCanvasElement) => {
    ...
    drawing.redraw(canvas, parentRegion, childRegions, zoom, pan, selectedChildId, colorSettings);
  }, [drawing, parentRegion, childRegions, zoom, pan, selectedChildId, colorSettings]);
